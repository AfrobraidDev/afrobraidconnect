...
export default function ServiceCard({
  ...
  const handleClick = () => {
    addToRecentlyViewed({ image, title, rating, reviews, location, services });
  };

  return (
    <Card
      onClick={handleClick}
      className="rounded-lg overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 group p-0 flex flex-col cursor-pointer"
    >
      {/* Image */}
      ...
      <CardContent className="p-4 space-y-1 flex-grow">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          ...
          {services.length > 4 && (
            <Badge
              variant="outline"
              className="rounded-full border px-3 py-1 text-xs font-normal text-gray-500"
            >
              +{services.length - 4}
            </Badge>
          )}
